...
func QueryPrometheus(host, bucketName, instance string, startTime, endTime time.Time) (int64, error) {
	...
	if rcvdValues[0] <= 0 || rcvdValues[1] <= 0 {
		log.Printf("[Warning] The metrics retrieved by vector-metrics are less than or equal to 0. bucket: %s\n", bucketName)
		log.Printf("received bytes: {startTime: {time: %v, value: %v}, endTime: {time: %v, value: %v}}\n", startTime.Format(timeFormat), rcvdValues[0], endTime.Format(timeFormat), rcvdValues[1])
		return 0, nil
	}
	...
	log.Printf("bucket: %v, received bytes in duration: %v, sent bytes in duration: %v\n", bucketName, receivedDiff, sentDiff)
	log.Printf("received bytes: {startTime: {time: %v, value: %v}, endTime: {time: %v, value: %v}}\n", startTime.Format(timeFormat), rcvdValues[0], endTime.Format(timeFormat), rcvdValues[1])
	log.Printf("sent bytes: {startTime: {time: %v, value: %v}, endTime: {time: %v, value: %v}}\n", startTime.Format(timeFormat), sentValues[0], endTime.Format(timeFormat), sentValues[1])
